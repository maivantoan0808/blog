@foreach($comments as $comment)
    <div class="card-comment">
	    ...
	    <img class="img-circle img-sm" src="/upload/users/{{$comment->user->avatar}}" alt="User Image">
        	...
        	@if (Auth::check())
	        	<div class="card-footer">
	               <!-- <form id="reply-form{{ $comment->id }}" method="post" action="{{ route('posts.comments.comments.store', [$post->id, $comment->id]) }}">
				        ...
				        <div class="img-push">
			                <input type="text" class="form-control form-control-sm" onclick="submitChildComment({{ $comment->id}})" placeholder="Press enter to post comment" name="message{{ $comment->id }}" id="child-comment{{ $comment->id }}" placeholder="Your Reply"  required>
				        </div>
			        <!-- </form> -->
			        <!--  </form> -->
		        </div>
